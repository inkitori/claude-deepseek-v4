...
def _v4_nan_tripwire(name: str, x: jnp.ndarray, layer_idx, position) -> None:
    ...
    so XLA cannot elide donated kv_cache writes (S1 heisenbug — empty
    callbacks alone aren't enough; only effectful sites paired with full-
    array reductions force materialization). Prints reductions when
    `V4_DECODE_NAN_TRIPWIRE=1` (set at process start)."""
    if x.size == 0:
