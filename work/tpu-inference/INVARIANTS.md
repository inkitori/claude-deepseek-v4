...
- I39. **e4m3fn 256-byte cast: numpy reference and torch's `.float()` cast agree bit-for-bit** on every one of the 256 possible input bytes. Non-NaN values match by uint32 reinterpretation; NaN positions match exactly the FN spec's `{0x7F, 0xFF}`. Locks our independent decoder against torch's native cast at the byte boundary.
...
## MoE forward
- I43. **Vectorized dense MoE is mathematically equivalent to top-k routing.** The forward stacks per-expert weights into `[E, ...]` tensors and uses a per-(token, expert) routing weight built from `one_hot(indices) * weights`, which is non-zero only for the top_k experts the gate picked. Output is byte-identical (maxabs=0) to the per-expert reference loop on a synthetic fixture across 5 seeds (3 top-k + 2 hash-routing). FLOP cost is the same as the per-expert loop — true sparse dispatch via `kernels/megablox/gmm.py` (backlog B2) would be ~32× cheaper for top_k=8 / E=256. The original 4 GiB all-gather caveat is resolved: per-(layer, wname) groups are pre-stacked into single E-sharded `[E, inter, dim]` jax.Arrays inline at load time (`deepseek_v4.py::_maybe_consolidate`), which eliminates the per-call all-to-all storm and dropped `jit_run_model` from 103k → 47k optimized HLO instructions.
