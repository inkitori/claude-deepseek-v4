...
`/home/<user>/claude-deepseek-v4/`. Key paths (relative to repo root):
  ...
  - `~/.cache/huggingface/hub/`  — when the GCS bucket is mounted via
    `scripts/mount_gcs.sh`, the real V4-Flash checkpoint resolves under
    here. Tier 8 needs this mount; Tiers 1–7 do NOT.
...
If the directory doesn't resolve, the user hasn't run the host's
`scripts/mount_gcs.sh`. Write `T8-mount-missing` to BLOCKERS.md and skip
Tier 8; do NOT try to mount it yourself (it requires bucket creds in
.env — host-side concern).
...
Synthetic fixtures live at `work/scratch/tiny_v4_*`. If absent, the host
hasn't run the fixture-build helper (`scripts/make_tiny_v4_checkpoint.py`);
write `Txx-fixtures-missing` to BLOCKERS.md and continue with non-
dependent work — do NOT generate them yourself unless the script exists
and the user explicitly authorized it.
