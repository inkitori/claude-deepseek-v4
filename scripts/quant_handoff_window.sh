#!/usr/bin/env bash
# quant_handoff_window.sh — hand off to a FRESH-context Claude Code session in a
# NEW tmux window of the CURRENT tmux session, for the QUANTIZED-WEIGHT-LOADING
# campaign (v6e-16). Retarget of perf_handoff_window.sh.
#
...
# the markdown lean, refreshes HANDOFF_QUANT.md, commits+pushes, then runs THIS
...
# with EMPTY context, auto-loads CLAUDE.md, and is handed scripts/quant_loop_prompt.txt.
#
# Why a new window (not `claude -p` headless like quant_session_loop.sh): an
...
#
# Usage (from inside tmux):  scripts/quant_handoff_window.sh
# Stop the chain: just don't call it (or `touch /tmp/quant_loop_stop`, which the
...
# _QUANT_LAUNCH=1) so there is no second script and no inline-quoting of the
# multi-line prompt through tmux.
if [[ "${_QUANT_LAUNCH:-}" == "1" ]]; then
  # CRITICAL: clear the flag BEFORE exec'ing claude. `exec` inherits the env, so a
  # leaked _QUANT_LAUNCH=1 persists in the spawned session, is inherited by every Bash
...
# Honour the same stop file the headless wrapper uses, so `touch /tmp/quant_loop_stop`
...
# session running.
tmux new-window -d -t "$SESSION:" -n "$WIN" -c "$REPO_ROOT" "_QUANT_LAUNCH=1 '$SELF'" </dev/null >/dev/null 2>&1
echo "[handoff] fresh max-thinking QUANT session launched in tmux session '$SESSION', window '$WIN' (background)."
